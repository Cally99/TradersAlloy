...
export default Vue.extend({
    ...
    methods: {
        click_q_latest() {
            ...
            this.t_latest = false
            this.t_chart = false
            this.params.context.componentParent.setFiltergp_q_latest();
        },
        click_q_chart() {
            this.q_latest = false
            this.q_chart = true
            ...
            this.t_latest = false
            this.t_chart = false
            this.params.context.componentParent.setFiltergp_q_chart();
        },
        click_y_latest() {
            this.q_latest = false
            this.q_chart = false
            ...
            this.t_latest = false
            this.t_chart = false
            this.params.context.componentParent.setFiltergp_y_latest();
        },
        click_y_chart() {
            this.q_latest = false
            this.q_chart = false
            ...
            this.t_latest = false
            this.t_chart = false
            this.params.context.componentParent.setFiltergp_y_chart();
        },
        click_t_latest() {
            this.q_latest = false
            this.q_chart = false
            ...
            this.t_latest = true
            this.t_chart = false
            this.params.context.componentParent.setFiltergp_t_latest();
        },
        click_t_chart() {
            this.q_latest = false
            this.q_chart = false
            ...
            this.t_latest = false
            this.t_chart = true
            this.params.context.componentParent.setFiltergp_t_chart();
        },
        onHistogramSliderMounted() {
            if (this.params.context.componentParent.filters.filter) {
                ...
                this.range.gp_min = this.min_value = filter['gp'] ? filter['gp'].min : -500;
                this.range.gp_max = this.max_value = filter['gp'] ? filter['gp'].max : 300;
        ...
        onFinish(event) {
            this.range.gp_min = event.from;
            this.range.gp_max = event.to;
        ...
        doesFilterPass(params) {
            if (this.q_latest) {
                if (params.data.gp_q_latest) {
                    const q_latest = parseInt(params.data.gp_q_latest);
                    return q_latest >= this.range.gp_min && q_latest <= this.range.gp_max
                }
            } else if (this.q_chart) {
                if (params.data.gp_q_array) {
                    const q_array = params.data.gp_q_array;
                    const index = q_array.length==0? 0 : q_array.length-1
                    return q_array[index] >= this.range.gp_min && q_array[index] <= this.range.gp_max
                }
            } else if (this.y_latest) {
                if (params.data.gp_y_latest) {
                    const y_latest = parseInt(params.data.gp_y_latest);
                    return y_latest >= this.range.gp_min && y_latest <= this.range.gp_max
                }
            } else if (this.y_chart) {
                if (params.data.gp_y_array) {
                    const y_array = params.data.gp_y_array;
                    const index = y_array.length==0? 0 : y_array.length-1
                    return y_array[index] >= this.range.gp_min && y_array[index] <= this.range.gp_max
                }
            } else if (this.t_latest) {
                if (params.data.gp_t_latest) {
                    const t_latest = parseInt(params.data.gp_t_latest);
                    return t_latest >= this.range.gp_min && t_latest <= this.range.gp_max
                }
            } else {
                if (params.data.gp_t_array) {
                    const t_array = params.data.gp_t_array;
                    const index = t_array.length==0? 0 : t_array.length-1
                    return t_array[index] >= this.range.gp_min && t_array[index] <= this.range.gp_max
                }
            }
        },
        isFilterActive() {
