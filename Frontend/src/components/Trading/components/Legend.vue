<template>
  <div class="trading-vue-legend" v-bind:style="calc_style">
    ...
    >
      <!-- <span class="t-vue-title" :style="{ color: common.colors.colorTitle }">{{common.title_txt}}</span> -->
      <span class="ohlcv-color">O</span>
      <span class="t-vue-lspan">{{ohlcv[0]}}</span>
      <span class="ohlcv-color">H</span>
      <span class="t-vue-lspan">{{ohlcv[1]}}</span>
      <span class="ohlcv-color">L</span>
      <span class="t-vue-lspan">{{ohlcv[2]}}</span>
      <span class="ohlcv-color">C</span>
      <span class="t-vue-lspan">{{ohlcv[3]}}</span>
      <span class="ohlcv-color">V</span>
      <span class="t-vue-lspan">{{ohlcv[4]}}</span>
    </div>
    <div class="t-vue-ind" v-for="(ind,index) in indicators" :key="index">
      <div v-if="ind.s" style="display:flex">
      ...
      return this.json_data
        ...
        .map(x => {
          ...
          return {
            ...
            s:"show" in x.settings ? x.settings.show:true
          };
        });
    },
    calc_style() {
