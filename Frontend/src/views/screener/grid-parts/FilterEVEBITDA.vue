<template lang="pug">
    div(class="grey7 pa-6")
        v-container(class="ma-0 pa-0" style="width:350px;")
            ...
            FilterViewRadioGroup(filterGroup="evEbitDaGroup" defaultSelected="q_latest")
</template>
...
export default Vue.extend({
    ...
    mounted() {
        this.is_filter = true;
        this.data = [
            {x0:-1000,frequency:16, display:'< 0'},
            {x0:-0.001,frequency:40, display:'0'},
            {x0:10,frequency:85, display:'10'},
            {x0:20,frequency:95, display:'20'},
            {x0:30,frequency:101, display:'30'},
            {x0:40,frequency:96, display:'40'},
            {x0:50,frequency:60, display:'50'},
            {x0:60,frequency:50, display:'60'},
            {x0:70,frequency:36, display:'70'},
            {x0:80,frequency:32, display:'80'},
            {x0:90,frequency:28, display:'90'},
            {x0:100,frequency:15, display:'100'},
            {x0:110,frequency:20, display:'110'},
            {x0:120,frequency:32, display:'120'},
            {x0:140,frequency:14, display:'140'},
            {x0:160,frequency:22, display:'160'},
            {x0:180,frequency:16, display:'180'},
            {x0:200,frequency:15, display:'200'},
            {x0:250,frequency:71, display:'> 250'},
        ];
    },
    beforeMount() {
    },
    ...
    methods: {
        ...
        click_btn(category, type) {
            ...
            this.params.context.componentParent.setFilterEVEBITDA(category, type === 'chart' ? 'array' : type);
        },
        ...
        doesFilterPass(params) {
            ...
            if (this.range.ev_ebitda_min == this.data[0].x0 && this.range.ev_ebitda_max == this.data[this.data.length-1].x0) {
                return true;
            }
        ...
        canFilter(val) {
            if (val) {
                if (this.params.context.componentParent.filters.filter) {
                    ...
                    this.range.ev_ebitda_min = filter['ev_ebitda_q_latest'] && filter['ev_ebitda_q_latest'].min ? filter['ev_ebitda_q_latest'].min : this.data[0].x0;
                    this.range.ev_ebitda_max = filter['ev_ebitda_q_latest'] && filter['ev_ebitda_q_latest'].max ? filter['ev_ebitda_q_latest'].max : this.data[this.data.length-1].x0;
                } else {
                    this.range.ev_ebitda_min = this.data[0].x0;
                    this.range.ev_ebitda_max = this.data[this.data.length-1].x0;
                }

                const minInx = this.data.findIndex(itr => itr.x0 == this.range.ev_ebitda_min);
                const maxInx = this.data.findIndex(itr => itr.x0 == this.range.ev_ebitda_max);
                this.range.displayFrom = this.data[minInx].display;
                this.range.displayTo = this.data[maxInx].display;
                this.range.init_from = this.data[0].x0;
                this.range.init_to = this.data[this.data.length-1].x0;
