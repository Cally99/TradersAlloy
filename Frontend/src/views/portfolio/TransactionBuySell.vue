    ...
    methods: {
        ...
        async loadRowData() {
            ...
            if(this.competition_user_account_id === -1) {
                final_array = this.getTx.filter((tx) => tx.user_account_id !== null);
                ...
                final_array.map((item, i) => {
                    if (this.getAllStocks.find((s) => s.stock_id === item.stock_id)) {
                        ...
                        let is_ignored = this.$store.getters.getUserAccounts.find((a) => a.user_account_id === item.user_account_id).is_ignored;
                        let is_selected = this.$store.getters.getUserAccounts.find((a) => a.user_account_id === item.user_account_id).is_selected;
                        ...
                        if ((item.tx_type === 'SELL' || item.tx_type === 'BUY') && !is_ignored && is_selected) {
        ...
        async clickedInsertNewTransaction() {
            ...
            const amount = Math.abs(this.quantityTextField) * Math.abs(this.priceTextField);
            ...
            const insertObject = {
                ...
                commission: this.commissionTextField !== "" ?  this.commissionTextField : 0,
                currency: currency,
            ...
            let account = this.getUserAccounts.find((a) => a.user_account_id === insertObject.user_account_id);

            if(insertObject.tx_type === "SELL") {
                account.cash += amount;
            } else {
                account.cash -= amount;
            }
            ...
            var tradeObject = {
                user_id: userId,
                stock_id: stock_id,
                ticker: this.tickerSelection,
                instrument_type: 'stock',
                user_account_id: this.accountItemsSelectField
            }
            if (this.transactionTypeSelection === 'BUY') {
                tradeObject.entry_price = Math.abs(this.priceTextField);
                tradeObject.entry_date = this.dateTextField;
                tradeObject.entry_qty = Math.abs(this.quantityTextField);
            }
            else {
                tradeObject.exit_price = Math.abs(this.priceTextField);
                tradeObject.exit_date = this.dateTextField;
                tradeObject.exit_qty = Math.abs(this.quantityTextField);
            }
            await ApiService.actionUserTrade(tradeObject);
        ...
        async clickSaveButton(row) {
            ...
            const newTx = updateSpecificRow;
            const oldTx = this.transaction_array_copy.find((tx) => tx.tx_id === updateSpecificRow.tx_id);

            const amountNew = parseInt(newTx.qty) * newTx.price;
            const amountOld = parseInt(oldTx.qty) * oldTx.price;
            const amountDiff = amountNew - amountOld

            // console.log(amountNew);
            // console.log(amountOld);
            ...
            let account = this.getUserAccounts.find((a) => a.user_account_id === user_account_id);

            if(tx.tx_type === "SELL") {
                account.cash += amountDiff;
            } else {
                account.cash -= amountDiff;
            }
        ...
        async clickRemoveButton(row) {
            ...
            const amount = Math.abs(rowNodeDataDelete.qty) * Math.abs(rowNodeDataDelete.price);

            let account = this.getUserAccounts.find((a) => a.user_account_id === rowNodeDataDelete.user_account_id);

            if(rowNodeDataDelete.tx_type === "SELL") {
                account.cash -= amount;
            } else {
                account.cash += amount;
            }
