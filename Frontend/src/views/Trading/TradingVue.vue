...
<script>
import Const from "@/plugins/stuff/constants.js";
import Chart from "@/components/Trading/Chart.vue";
import Toolbar from "@/components/Trading/Toolbar.vue";
import DataToolbar from "@/components/Trading/DataToolbar.vue";
...
export default {
  ...
  data() {
    return { reset: 0, dateRange: "", interval: "", lineType: "" };
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    set_loader(dc) {
      ...
      if (dc)
        this.$refs.chart.$on("range-changed", r =>
          dc.range_changed(r, this.$refs.chart.interval)
        );
    }
  },
  mounted() {
    this.$emit("chnInterval", "");
  }
};
</script>
