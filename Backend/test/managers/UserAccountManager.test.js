...
const userAccountManager = require('./../../server/managers/UserAccountManager');
...
describe('UserAccountManager', () => {
    ...
    it('user accounts should be of type array', async () => {
        const userAccounts = JSON.parse(JSON.stringify(await userAccountManager.getUserAccounts(testUser.user_id)));
    ...
    it('should update one account for the user', async () => {
        const userAccountBeforeUpdate = JSON.parse(JSON.stringify(await userAccountManager.getUserAccount('184-1')));
        ...
        await userAccountManager.updateUserAccount(accountObject);

        const userAccountAfterUpdate = JSON.parse(JSON.stringify(await userAccountManager.getUserAccount('184-1')));
    ...
    it('should delete one account for the user', async () => {
        const userAccount = JSON.parse(JSON.stringify(await userAccountManager.getUserAccount('184-1')));
        const userAccountsBeforeDelete = JSON.parse(JSON.stringify(await userAccountManager.getUserAccounts(testUser.user_id)));

        await userAccountManager.deleteUserAccount(userAccount.user_account_id);

        const userAccountsAfterDelete = JSON.parse(JSON.stringify(await userAccountManager.getUserAccounts(testUser.user_id)));
