...
const CompanyWebcastManager = require("../managers/CompanyWebcastManager.js");
...
module.exports = {
    async list() {
        return await CompanyWebcastManager.list();
    },
    async getOneCompanyWebcast(company_id){
        return await CompanyWebcastManager.getOneCompanyWebcast(company_id);
    },
    async get_initial() {
        companies = [];
        webcasts = [];
        ...
        scrapped_array = await CompanyWebcastManager.list();

        healthCheck.info(`Webcast: start scrapping...`);
                        ...
                        if (await page.waitForSelector(expandableSelector) !== null) {
                            ...
                            for (let x = 0; x < webcasts.length; x++) {
                                if (webcasts[x].company_id == company.company_id && webcasts[x].publish_date == date && webcasts[x].nth_today == nth_today) {
                                    healthCheck.info(`Webcast: end scrapping...`);
                                    return;
                                }
                            }
                            ...
                            if (webcast) {
                                ...
                                healthCheck.info(`Webcast: added new record... ${company.company_id}`);
                                healthCheck.info(`Webcast: total count of new records... ${scrapped_array.length}`);
                            }
                        }
                    }
        ...
        healthCheck.info(`Webcast scrapping ended... FINALLY===`);
    }
};